...
- Added a reusable fail-closed private-to-public readiness runbook plus an `ai-dev-starter`-specific audit record. Publication now requires full-history secret/metadata review, GitHub-hosted discussion and Actions-log/artifact review, explicit license/redistribution decisions, public-fork workflow review, a human visibility-change gate, and authoritative post-publication protection verification. The repository remains private and the initial audit is `BLOCKED`; this change grants no publication authority. See Issue #33.
