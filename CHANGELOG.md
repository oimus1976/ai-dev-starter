...
- Added fail-closed post-merge safe cleanup as a separate protected effect after verification. The cleanup planner performs its own authenticated GitHub PR read, defaults to dry-run, revalidates before `--execute`, uses normal worktree removal and exact-lease remote deletion, supports linked and single-checkout layouts, leaves remote deletion off by default, and distinguishes pre-effect `BLOCKED` from post-effect `INCOMPLETE`. After L2 review exposed non-atomic occupancy and remote-reappearance races in raw ref CAS, v1 was narrowed: local topic branches are auto-deleted only when the exact PR head is already in fresh canonical history and Git's normal `branch -d` accepts the deletion; squash/rebase-style local topic refs and stale remote-tracking refs are retained rather than force/CAS-deleted. The existing verifier remains non-destructive. See Issue #12 and ADR-0001.