...
- Added a reusable fail-closed private-to-public readiness runbook plus an `ai-dev-starter`-specific audit record. Publication now requires full-history secret/metadata review, GitHub-hosted discussion and Actions-log/artifact review, explicit license/redistribution decisions, public-fork workflow review, a human visibility-change gate, and authoritative post-publication protection verification. The human owner selected the MIT License and a root `LICENSE` file was added; publication remains unauthorized until the final exact-head refresh and human Ready/visibility gates complete. See Issue #33.
