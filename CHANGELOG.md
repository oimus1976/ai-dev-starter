...
- Added authoritative repository-wide branch cleanup as a separate fail-closed phase after a real `life-dashboard` cleanup exposed partial prior deletion, misleading native stderr rendering, stale deletion assumptions, closed-unmerged branches that required successor review, and a no-PR long-lived branch that had to be retained. The new audit helper uses current GitHub state as authority, separates inventory/classification/plan/execute/verification, treats native exit status rather than stderr as command authority, requires exact reviewed SHA for closed-unmerged deletion, and refetches GitHub to prove zero residual targets before remote cleanup can be reported complete. Existing one-PR local closeout/worktree cleanup authority remains unchanged. See Issue #20.
